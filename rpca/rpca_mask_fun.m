function [Parms,wavoutE,wavoutA]=rpca_mask_fun(wavinA,wavinE,wavinmix,parm)
    %% parameters
   ...
   %% use inexact_alm_rpca to run RPCA
    ...
    wavwrite(wavoutE,Fs*1.5,[outputname,num2str(1.5),'_E']);
    wavwrite(wavoutE,Fs*2,[outputname,num2str(2),'_E']);
    wavoutA=wavoutA/max(abs(wavoutA));
    wavwrite(wavoutA,Fs*1.5,[outputname,num2str(1.5),'_A']);
    wavwrite(wavoutA,Fs*2,[outputname,num2str(2),'_A']);
%     %% evaluate
